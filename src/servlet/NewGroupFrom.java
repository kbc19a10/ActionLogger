...
@WebServlet("/NewGroupFrom")
public class NewGroupFrom extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// addUserForm.jspへ
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/jsp/newGroupForm.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 名前
			newgroup.setGname(req.getParameter("gname"));

			// actionオブジェクトをセッションスコープに一旦保存（DBに入れるのはConfirmの後）
			session.setAttribute("groupToAdd", newgroup);

			// 確認画面にリダイレクト(jsp)
			// addActionConfirmへ
			RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/jsp/newGroupConfirm.jsp");
			dispatcher.forward(req, resp);
